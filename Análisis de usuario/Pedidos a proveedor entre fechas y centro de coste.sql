...
select 
   ...
   ,convert(varchar(10), box.dataobra, 103) as 'Fecha pedido Proveedor'
   ,convert(varchar(10), box.dataobra, 103) as 'Fecha pedido cliente'
...
left join 
   -- En este caso, si sacamos pedidos a proveedor (ndos=2) el destino tiene en oobistamp el bistamp del origen (pedido de cliente).
   bi bix (nolock) on bix.bistamp=bi.oobistamp 
left join 
